...
def load_dataset(
        config: Any,
        device: torch.device,
) -> Union[CUDAPrefetcher, CUDAPrefetcher]:
...
def build_model(
        config: Any,
        device: torch.device,
) -> Union[nn.Module, nn.Module, nn.Module]: # [nn.Module, nn.Module or Any, nn.Module]
...
def define_loss(config: Any, device: torch.device) -> Union[nn.MSELoss, model.ContentLoss, nn.BCEWithLogitsLoss]:
...
def define_optimizer(g_model: nn.Module, d_model: nn.Module, config: Any) -> Union[optim.Adam, optim.Adam]:
...
def define_scheduler(g_optimizer: optim.Adam, d_optimizer: optim.Adam, config: Any) -> Union[lr_scheduler.MultiStepLR, lr_scheduler.MultiStepLR]:
